---
title: A supervised learning strategy for detecting rare classes using data-centric
  ensembling
author: Danny Morris
date: '2020-06-07'
output: 
  blogdown::html_page:
    toc: true
    highlight: pygments
slug: a-supervised-learning-strategy-for-detecting-rare-classes-using-data-centric-ensembling
categories:
  - Data Science
  - Machine Learning
  - Outlier Analysis
...
SET stratified training and testing data

SET number of ensemble iterations

FOR each ensemble iteration 
  SET downsampled training data 
  TRAIN model on downsampled data
  FOR each instance in the testing data
    COMPUTE positive class predicted probability
  ENDFOR
ENDFOR

FOR each instance in the testing data
  COMPUTE average positive class predicted probability
ENDOR
...
SET number of cross validation iterations

FOR each cross validation iteration
  SET stratified training and testing splits
...
COMPUTE average performance metric across all cross validation iterations
```
...
SET training and inference data
...
FOR each ensemble iteration 
  SET downsampled training data 
  TRAIN model on downsampled data
  FOR each instance in the inference data
    COMPUTE positive class predicted probability
  ENDFOR
ENDFOR
